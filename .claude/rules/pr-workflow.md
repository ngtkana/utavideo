...
- 対応するissueがある変更は、PR本文に必ず `Closes #<番号>`（複数あればissueごとに1行）を書く。書き忘れると、マージしてもissueが自動でクローズされず、対応済みのissueが開いたまま放置される（orphan issue）。PRを開いた後に気づいたら `gh pr edit <番号> --body-file <ファイル>` で本文に追記する
- マージ後は、該当issueが実際にクローズされたか `gh issue view <番号> --json state` で確認する（`Closes` の書き方が不正だと自動クローズされないことがある）
