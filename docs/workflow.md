...
音源を差し替えるときは、新しいバージョン名のファイル（`song-v1.1.wav`）を置いて `audio.file` を書き換えます。ファイル名の `vX.Y` が、`release` で付くバージョンになります（規則は [project-layout.md](project-layout.md#名前の付け方)）。
...
```sh
utavideo release                  # release/song-vX.Y.mp4（バージョンは音源のファイル名から）
