...
- 1区間 = スタイル `Short` の**コメント行**1本（Dialogue 行だと画面に出てしまい、`check` がエラーにします）
- 区間の開始・終了 = その行の開始・終了時刻。Aegisub 上での行の長さが、そのまま区間の長さになります
- 本文 = `[[shorts]]` の `name` と、大文字小文字も含めて一致させる
- 制約: 同じ `name` の行はちょうど1つ（無い・2つ以上あるとエラー）、終了は開始より後、終了は音源の長さ以内（超えるとエラー）
- 区間の頭は、歌い出しの少し前に置く（区間の端は `vertical.audio_fade_ms` でフェードし、フェードインの間は音が小さいため）
