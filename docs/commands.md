...
- `check`・`preview-bg`・`build`・`overlay` には libass 付きの ffmpeg と ffprobe が必要です（[動作環境](../README.md#動作環境)）。ffmpeg・ffprobe が無ければ、何を入れればよいかを表示して始めに止まります。libass が無いことは、`preview-bg`・`build`・`overlay` では書き出す前に止まり、`check` では[エラー](#検査項目)として他の検査結果と一緒に出ます
