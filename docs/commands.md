...
- `check`・`preview-bg`・`build`・`overlay` には libass 付きの ffmpeg と ffprobe が必要です（[動作環境](../README.md#動作環境)）。始めに調べ、足りなければ何を入れればよいかを表示して、書き出す前に止まります
...
| 実行環境 | ffmpeg・ffprobe が無い、ffmpeg で `subtitles` フィルタ（libass）が使えない |
