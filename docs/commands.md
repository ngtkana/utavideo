...
- `check`・`preview-bg`・`build`・`overlay`・`thumbnail`・`shorts`・`description`・`release`・`vertical-ass` は曲フォルダで実行します。`-C <曲フォルダ>`（`--project`）で指定でき、省略するとカレントディレクトリから親へ向かって `utavideo.toml` を探します
- `check`・`preview-bg`・`build`・`overlay`・`thumbnail`・`shorts` には ffmpeg と ffprobe が必要です
...
| 区間に入る縦の歌詞が、本編の .ass と食い違う（[本編との突き合わせ](#本編との突き合わせ)） | 警告 | ○ | ○ | X だけ | － |
...
本編の歌詞を直して縦用 .ass を直し忘れたときに、警告で気づけるようにします。縦で改行・位置・大きさ・スタイルを変えただけなら、何も出ません。本編のスタイルの変更（フォント・色）は比べません。

| 比べるもの | 行 |
|---|---|
| 本編 | 本編の .ass の Dialogue 行 |
| 縦 | 縦用 .ass の歌詞の行（スタイルが `Short` でも `Vertical` で始まるものでもない行）。Dialogue 行とコメント行の両方 |
| 比べない | 図形（`\p1` など）の行 |
| 文字から除くもの | タグ（`{...}`）・`\N`・`\n`・`\h`・空白（全角の空白も） |

1. 縦の行を、本編の行のうち**重なる時間がいちばん長い**行に割り当てます。同じ長さなら、文字を含む行、文字が同じ行、スタイルが同じ行、レイヤーが同じ行、まだ割り当てていない行、.ass の先の行の順に選びます
2. 区間に入る本編の行ごとに、割り当てた縦の行を見ます

| 割り当てた縦の行 | 結果 |
|---|---|
| 無い | 警告（対応する行が無い）。本編の行の文字が空なら出さない |
| すべてコメント行 | 縦で出さないことにした行とみなし、何も出さない |
| Dialogue 行がある | Dialogue 行の文字を開始の順につなぎ、本編の文字と違えば警告（両方の文字を表示）。Dialogue 行の最初の開始・最後の終わりと、本編の行の開始・終わりの差が **10 ミリ秒**を超えれば警告 |
...
| 本編の行の時刻だけ変えた | 警告（時刻） |
| 縦で改行・位置・大きさ・スタイルを変えた、1行を2つの行に分けた | 何も出ない |
| 縦で行を出さないことにした | その行をコメント行にすれば、何も出ない |
| 本編の2行を、縦で1行にまとめた | 警告（片方の行は対応する行が無い、もう片方は文字と時刻） |
