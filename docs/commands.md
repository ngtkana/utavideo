...
- `[inst.score]` を設定すると、MuseScore 4 で作った楽譜（`.mscz`）を、音源のテンポに合わせて画面上を横スクロールで表示します（[config-reference.md](config-reference.md#score)）。MuseScore 4 のインストールが要ります（見つからない場所は環境変数 [`UTAVIDEO_MUSESCORE`](config-reference.md#環境変数) で指定できます）。楽譜と音源の食い違いの検知はまだ対応していません
