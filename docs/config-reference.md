...
- macOS: `/System/Library/Fonts`（`Supplemental` はこの下なので一緒に探します）、`/Library/Fonts`、`~/Library/Fonts`（`/Network/Library/Fonts` は、マウントされていないと遅くなるため含みません。必要なら `font_dirs` に書き足してください）
