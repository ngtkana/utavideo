...
| `layout` | `"reframe"` / `"blur"` | `"blur"` | 画面の作り方（[commands.md](commands.md#shorts)）。`reframe` は背景を縦に切り取って縦用 .ass の行を重ね、`blur` は本編の映像をぼかした帯の上に置く。`utavideo preview-bg` が歌詞の行を写すかと、縦の下敷きの画面は、`[[shorts]]` があればそちらの `layout` で決まる |
| `lyrics` | パス | `"src/vertical.ass"` | 縦用 .ass。`utavideo preview-bg` が無ければここに作る。ショートの区間もここに書く（[`[[shorts]]`](#shorts)） |
| `focus` | `[x, y]`（0〜1 の数） | `video.focus` | 背景を縦に合わせるとき、どこを基準にするか（意味は `[video]` の `focus` と同じ）。縦の下敷きと `shorts` に使う（`blur` では上下の帯の切り取りに使う） |
...
| `layout` | `"reframe"` / `"blur"` | `vertical.layout` | このショートだけ、画面の作り方を変える。`preview-bg` の縦用 .ass の自動生成・下敷きも、この表で実際に使う作り方に合わせる |
