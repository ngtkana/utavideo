...
背景の上に重ねる画像・GIF・アルファ付き動画です。1個の素材につき1つ書きます。本編・`preview-bg`（本編・縦の両方）・サムネイルで共通に使います（出力ごとの上書き・除外はまだできません）。
...
- `.webm` はアルファ付き VP9 を想定する。書き出す側で `-auto-alt-ref 0` を付けないとアルファが欠けることがある
