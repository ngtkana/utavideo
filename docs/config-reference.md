...
| `layout` | `"reframe"` / `"blur"` | `vertical.layout` | このショートだけ、画面の作り方を変える。`vertical-ass`・`preview-bg --vertical` も、この表で実際に使う作り方に合わせる |
...
`utavideo inst` が書き出す、歌唱練習用の動画に表示する文字です。歌詞は描かず、この文字だけを表示します。
