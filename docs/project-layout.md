...
`[[thumbnails]]`・`[[shorts]]` は `name` で宣言し、`utavideo.toml` の中に永続する識別子を持ちます。一方 `inst`（歌唱練習用）は `--keys` で都度指定するだけで、`utavideo.toml` に識別子を持ちません。これは「`release/` に残る、公開する成果物だけが `utavideo.toml` に永続識別子を持つ」という決まりで、`inst` は公開しない使い捨ての成果物なので識別子を持ちません。新しい種類の成果物を足すときも、公開するかどうかでこの決まりに従ってください。
