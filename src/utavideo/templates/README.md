...
- [ ] 縦型のショートを作るなら、`utavideo.toml` に `[vertical]`（画面の作り方 `layout`）を書く → `utavideo preview-bg`（縦用 .ass を自動で作り、下敷きも書き出す）→ Aegisub で `src/vertical.ass` を組み、区間を置く → `utavideo.toml` に `[[shorts]]` → `utavideo shorts`
